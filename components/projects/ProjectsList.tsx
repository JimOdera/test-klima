...
const STATUS_STYLES: Record<StatusKey, { bg: string; text: string; progressBg: string; progressFill: string }> = {
    ...
    'Completed': { bg: 'bg-[#D1F3E4]', text: 'text-[#044D5E]', progressBg: '#044D5E', progressFill: '#044D5E' },
    ...
    return (
        <div className="relative w-11 h-11 flex items-center justify-center">
            <svg width="44" height="44" viewBox="0 0 44 44" className="-rotate-90">
                <circle cx="22" cy="22" r={radius} fill={circleFill} stroke={circleStroke} strokeWidth="4" />
                {!isFull && !isEmpty && (
                    <circle cx="22" cy="22" r={radius} fill="none" stroke={progressFill} strokeWidth="4"
                        strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" />
                )}
            </svg>
            <span className={`absolute text-[9px] font-bold ${labelColor}`}>{progress}%</span>
        </div>
    )
}
